...
def obter_metas():
    ...
    for linha in linhas:
        d = dict(linha)
        if d['tipo_meta'] == 'frequencia_semanal':
            cursor.execute("SELECT COUNT(*) FROM historico_treinos WHERE data_realizacao >= ?", (sete_dias_atras,))
            d['valor_atual'] = float(cursor.fetchone()[0])
            d['concluid'] = 1 if d['valor_atual'] >= d['valor_meta'] else 0
        elif d['tipo_meta'] == 'frequencia_total':
...
def obter_evolucao():
    ...
    cursor.execute("SELECT titulo, valor_atual, valor_meta, unidade, concluid, valor_inicial, tipo_meta FROM metas")
    ...
    for meta in metas_banco:
        ...
        if tipo_meta == 'frequencia_semanal':
            cursor.execute("SELECT COUNT(*) FROM historico_treinos WHERE data_realizacao >= ?", (sete_dias_atras,))
            v_atual = float(cursor.fetchone()[0])
            is_concluida = 1 if v_atual >= v_meta else 0
            progresso_porcentagem = round((v_atual / v_meta) * 100, 2) if v_meta > 0 else 0.0
        elif tipo_meta == 'frequencia_total':
            cursor.execute("SELECT COUNT(*) FROM historico_treinos")
            v_atual = float(cursor.fetchone()[0])
            is_concluida = 1 if v_atual >= v_meta else 0
            progresso_porcentagem = round((v_atual / v_meta) * 100, 2) if v_meta > 0 else 0.0
...
def obter_sugestoes():
    ...
    sete_dias_atras = (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d %H:%M:%S')
    cursor.execute("SELECT COUNT(*) FROM historico_treinos WHERE data_realizacao >= ?", (sete_dias_atras,))
    freq_semanal = cursor.fetchone()[0]

    if freq_semanal == 0:
        sugestoes.append("🚨 Você não registrou treinos nos últimos 7 dias. Que tal agendar uma sessão na agenda do Google hoje para voltar ao ritmo?")
    elif freq_semanal >= 4:
        sugestoes.append("🔥 Ritmo excelente! Você treinou {} vezes nos últimos 7 dias. Lembre-se de planejar dias de descanso para regeneração muscular.".format(freq_semanal))
    else:
        sugestoes.append("💪 Bom trabalho! Você acumulou {} treinos nesta semana. Mantenha o foco para fechar sua meta semanal.".format(freq_semanal))

    cursor.execute('''
        SELECT t.id, t.nome, MAX(h.data_realizacao) as ultima_vez
        FROM treinos t
        LEFT JOIN historico_treinos h ON t.id = h.id_treino
        GROUP BY t.id
    ''')
    status_treinos = cursor.fetchall()

    if status_treinos:
        treino_nunca_feito = [t for t in status_treinos if t['ultima_vez'] is None]
        if treino_nunca_feito:
            sugestoes.append("✏️ O treino '{}' está cadastrado, mas ainda não foi realizado nenhuma vez. Que tal dar o primeiro passo nele?".format(treino_nunca_feito[0]['nome']))
        else:
            status_treinos_ordenados = sorted(status_treinos, key=lambda x: x['ultima_vez'])
            mais_antigo = status_treinos_ordenados[0]
            
            data_dt = datetime.fromisoformat(mais_antigo['ultima_vez'].replace(" ", "T"))
            dias_sumido = (datetime.now() - data_dt).days
            
            if dias_sumido > 5:
                sugestoes.append("📅 O treino '{}' não é realizado há {} dias. Pode ser uma boa ideia priorizá-lo no seu próximo dia de academia para manter o equilíbrio corporal.".format(mais_antigo['nome'], dias_sumido))
